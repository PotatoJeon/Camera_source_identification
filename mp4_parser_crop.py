...
def parse_mvhd(mp4_file, current_box):
    ...
    #print(mvhd_info)
    video_attribute['vmhd_time_scale'] = mvhd_info['Time_Scale']
    video_attribute['vmhd_next_track_ID'] = mvhd_info['Next_Track_Id']
    return {}
...
def parse_CNDA(mp4_file, current_box):
    ...
    image_addr = "./in_video_jpg_file.jpg"
...
def make_rough_dict(mp4_file, box_list, dict):
    ...
    for key in dict.keys():
        #2024/04/02 M22_Nikon_Coolpix_S7000에서 free 박스가 있기도하고 없기도 하는 경우를 확인
        #2024/04/03 그래서 free 박스를 제거하도록 추가함
        if key in ['udta', 'edts', 'stco', 'co64', 'free']:
...
#원본 박스 구조를 출력하는 함수
def make_detail_dict(mp4_file, box_list, dict):
    keys = dict.keys()
...
file_addr = "D:/졸업논문/data_mp4_h265/F37_Apple_iPhone12/D37_L6S3C4.MOV"
    ...
    for file in files: #파일 넘어가는게 이 반복문
        ...
        if file in [".json" ,".AVI"] :
            ...
            if parsed_result[1]['codec'] == 'h.264':
                ...
                h264_vmhd_time_scale.append(str(parsed_result[1]['vmhd_time_scale']))
                h264_mdhd_time_scale.append(str(parsed_result[1]['mdhd_time_scale']))
                ...
                h265_vmhd_time_scale.append(str(parsed_result[1]['vmhd_time_scale']))
...
h264_video_info['h264_vmhd_time_scale'] = h264_vmhd_time_scale
...
h265_video_info['h265_vmhd_time_scale'] = h265_vmhd_time_scale
...
#h264_info.to_csv("./only_h264_result.csv")

h265_info = pd.DataFrame(h265_video_info)
h265_info.to_csv("./only_h265_final_result.csv")'''
